...
class Jcart {
	...
	* @return mixed
	*/
	private function add_item($id, $img, $name, $price, $qty = 1, $url) {
	...
	*/
	public function display_cart() {
		...
		echo tab(3) . "<h1> Shopping Cart </h1> \n";
		echo tab(3) . "<table id='shopping-cart-table' class='data-table cart-table'>\n";
		echo tab(4) . "<thead>\n";
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				echo tab(5) . "<tr>\n";
				//echo tab(6). "<td> Product Image</td>";
		...
		echo tab(4) . "<input type='$inputType' $src name='jcartUpdateCart' value='{$config['text']['update']}' class='jcart-button cartbtn' />\n";
